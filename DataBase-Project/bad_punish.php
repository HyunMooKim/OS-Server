...
if (mysqli_query($conn, "SELECT 1 FROM Post WHERE 작성자 = $UID LIMIT 1")) {
    $query = "UPDATE Post SET 글신고 = 0, 글내용 = '악질 유저 활동으로 판명하여 삭제하였습니다.' WHERE 작성자 = $UID";
...
if (mysqli_query($conn, "SELECT 1 FROM Comment WHERE 작성자 = $UID LIMIT 1")) {
    $query = "UPDATE Comment SET 댓글신고 = 0, 댓글내용 = '악질 유저 활동으로 판명하여 삭제하였습니다.' WHERE 작성자 = $UID";
    mysqli_query($conn, $query);
}
else{
	mysqli_query($conn, "rollback");
...
if (mysqli_query($conn, "SELECT 1 FROM Recomment WHERE 작성자 = $UID LIMIT 1")) {
    $query = "UPDATE Recomment SET 대댓글신고 = 0, 대댓글내용 = '악질 유저 활동으로 판명하여 삭제하였습니다.' WHERE 작성자 = $UID";
    mysqli_query($conn, $query);
}
else{
	mysqli_query($conn, "rollback");
